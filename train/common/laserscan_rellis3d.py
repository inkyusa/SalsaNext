...
class LaserScan:
    ...
    def open_scan(self, filename):
        ...
        points[:, 0] = -points[:, 0]
        points[:, 1] = -points[:, 1] #rellis-3d transform
    ...
    def do_range_projection(self):
        ...
        # get scan components
        scan_x = self.points[:, 0] #just for testing rellist3d kitti x = rellis -x
        scan_y = self.points[:, 1] #kitti y = rellis -y
        scan_z = self.points[:, 2]

        # get angles of all points
        yaw = -np.arctan2(scan_y, scan_x)
        pitch = np.arcsin(scan_z / depth)
        pitch_NaNs = np.isnan(pitch)
        pitch[pitch_NaNs] = 0
