...
class Parser():
  ...
  def __init__(self,
    ...
    # Data loading code
    self.train_dataset = Rellis3D(root=self.root,
    ...
    self.valid_dataset = Rellis3D(root=self.root,
    ...
    if self.test_sequences:
      self.test_dataset = Rellis3D(root=self.root,
  ...
  def to_original(self, label):
    ...
    return Rellis3D.map(label, self.learning_map_inv)

  def to_xentropy(self, label):
    # put label in xentropy values
    return Rellis3D.map(label, self.learning_map)

  def to_color(self, label):
    # put label in original values
    label = Rellis3D.map(label, self.learning_map_inv)
    # put label in color
    return Rellis3D.map(label, self.color_map)